...
async function apiCall(endpoint, method = 'GET', data = null) {
  try {
    ...
    if (!contentType || !contentType.includes('application/json')) {
      ...
      throw new Error(
        `Server returned HTML instead of JSON. Endpoint: ${endpoint}\nMake sure the backend server is running on port 3001`
      );
    }
...
async function loadAppointmentTreatments() {
  try {
    ...
    if (apptTreats.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="empty-state">No appointment treatments found</td></tr>';
      return;
    }

    tbody.innerHTML = apptTreats
      .map(
        at => `
            <tr>
...
async function loadAppointmentTreatmentData(apptTreatID) {
  try {
    ...
    document.getElementById('appointment-treatment-appointmentID').value =
      apptTreat.appointmentID || '';
    document.getElementById('appointment-treatment-treatmentID').value =
      apptTreat.treatmentID || '';
...
async function loadFinancialRecords() {
  try {
    ...
    if (records.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="empty-state">No financial records found</td></tr>';
      return;
    }
...
async function adminAction(action) {
  if (
    action === 'drop' &&
    !confirm(
      '⚠️ WARNING: This will delete all tables. This action cannot be undone. Are you absolutely sure?'
    )
  ) {
    return;
  }
