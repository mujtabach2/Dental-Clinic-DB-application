...
router.get("/:id", (req, res, next) => {
  try {
    const row = db.prepare("SELECT * FROM Appointment_Treatment WHERE apptTreatID=?").get(req.params.id);
...
router.post("/", (req, res, next) => {
  try {
    ...
    db.prepare("INSERT INTO Appointment_Treatment(apptTreatID,appointmentID,treatmentID,quantity) VALUES (?,?,?,?)")
...
router.put("/:id", (req, res, next) => {
  try {
    ...
    const info = db.prepare("UPDATE Appointment_Treatment SET appointmentID=?, treatmentID=?, quantity=? WHERE apptTreatID=?")
...
router.delete("/:id", (req, res, next) => {
  try {
    const info = db.prepare("DELETE FROM Appointment_Treatment WHERE apptTreatID=?").run(req.params.id);
    if (info.changes === 0) return res.status(404).json({ error: "Not found" });
    res.json({ ok: true });
  } catch (err) { next(err); }
});
